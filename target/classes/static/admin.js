...
function selectedOptions(selects) {

    let changeFinalRoles = []
    const selectedValues = [].filter.call(selects.options, option => option.selected)
        .map(option => option.text)
    if(selectedValues.length === 0) {
        changeFinalRoles.push({id: 1, role: "ROLE_USER"})
    }
    else {
        selectedValues.forEach((chosen) => {
            chosen === 'USER' ? changeFinalRoles.push({id: 1, role: "ROLE_USER"}) : changeFinalRoles.push({
                id: 2,
                role: "ROLE_ADMIN"
            })
        })
    }
    return changeFinalRoles
}
...
const editSelectRoles = document.querySelector('.editModalSelect')
...
on(document, 'click', '.editModalBtn', async e => {
    ...
    fetch(URL, {
        ...
        body: JSON.stringify({
            ...
            age: AgeEdit.value,
            roles: selectedOptions(editSelectRoles)
        })
    })
        .then(res => {
            res.json()
...
const createSelectRoles = document.querySelector('.createSelect')
...
on(document, 'click', '.createNewUser', async e => {
    ...
    fetch(URL, {
        ...
        body: JSON.stringify({
            ...
            age: ageCreate.value,
            roles: selectedOptions(createSelectRoles)
        })
    }).then(res => res.json())
        .then(user => {
